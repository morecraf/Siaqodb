    ...
    public class SiaqodbSync : IDisposable
    {
...
#endif
#if ASYNC
        public async Task<PushResult> PushAsync(IBucket bucket)
        {
            return await this.PushAsync(bucket, null);
        }
        public async Task<PushResult> PushAsync(IBucket bucket, IConflictResolver conflictResolver)
        {
            if (!Sqo.Internal._bs._ibs((Bucket)bucket))
            {
                throw new SiaqodbException("Bucket:" + bucket.BucketName + " is not syncable, set it via SiaqodbConfigurator.SetSyncableBucket(...) method");
            ...
            try
            {
                ...
                {
                    this.OnSyncProgress(new SyncProgressEventArgs("Uploading local changes..."));
                    var response = await httpClient.PutAsync(RemovePrefix( bucket.BucketName), changeSet);
                    this.OnSyncProgress(new SyncProgressEventArgs("Upload finished, build the result..."));
                    ...
                    conflicts = await ManageConflictsAsync(bucket, conflictResponses, changeSet, conflictResolver);
                    if (changeSet.ChangedDocuments != null)
                    {
                        syncStatistics.TotalChangesUploads = changeSet.ChangedDocuments.Count;
                    }
        ...
        private async Task<List<Conflict>> ManageConflictsAsync(IBucket bucket, IEnumerable<BatchItemResponse> conflictResponses, ChangeSet changeSet, IConflictResolver conflictResolver)
        {
            ...
            foreach (var conflictR in conflictResponses)
            {
                if (conflicts == null)
                    conflicts = new List<Conflict>();
                var liveVersion =await httpClient.GetAsync(RemovePrefix(bucket.BucketName), conflictR.Key);
                var version = liveVersion != null ? liveVersion.Version : null;
                var cf = new Conflict() { Key = conflictR.Key, Version = version, Description = conflictR.ErrorDesc };
                if (conflictResolver == null)
                    conflictResolver = new ServerWinResolver();
                // TODO avoid the extra call to the local database
                Document localVersion = bucket.Load(conflictR.Key);
                await KeepChangesByConflictConvensionAsync(bucket, localVersion, liveVersion, conflictR.Key, conflictResolver);
        ...
        private async Task KeepChangesByConflictConvensionAsync(IBucket bucket, Document localVersion, Document liveVersion, string key, IConflictResolver conflictResolver)
        {
            var winner = conflictResolver.Resolve(localVersion, liveVersion);
            var version = liveVersion != null ? liveVersion.Version : null;
            if (winner == null)//means the winner is the Deleted one
            {
                if (version != null)
                {
                    await httpClient.DeleteAsync(RemovePrefix(bucket.BucketName), key, version);
                }
                if (localVersion != null)
                {
                   Sqo.Internal._bs._de((Bucket)bucket,key, false);
            ...
            else
            {
                ...
                winner.Version = version;
                var resp = await httpClient.PutAsync(RemovePrefix(bucket.BucketName), winner);
                winner.Version = resp.Version;
                Sqo.Internal._bs._si((Bucket)bucket,winner,false);
            }
        }
...
#if ASYNC
        public async Task<PullResult> PullAsync(IBucket bucket, IConflictResolver conflictResolver)
        {
            return await this.PullAsync(bucket, null, conflictResolver);

        }
        public async Task<PullResult> PullAsync(IBucket bucket)
        {
            return await this.PullAsync(bucket, null, null);

        }
#endif

#if ASYNC
        public async Task<PullResult> PullAsync(IBucket bucket, Filter filter, IConflictResolver conflictResolver = null)
        {
            var pushResult = await PushAsync(bucket, conflictResolver);
            var syncStatistics = new PullStatistics();
            ...
            try
            {
                ...
                while (remainLimit > 0)
                {
                    remainLimit = 0;
                    this.OnSyncProgress(new SyncProgressEventArgs("Downloading batch #" + nrBatch + " ..."));
                    downloadedItems = await DownloadChangesAsync(bucket,filter, anchor, pushResult.UploadAnchor);
                    this.OnSyncProgress(new SyncProgressEventArgs("Batch #" + nrBatch + " downloaded, store items locally ..."));
...
#if ASYNC
        private async Task<ChangeSet> DownloadChangesAsync(IBucket bucket,  Filter query, string anchor,string uploadAnchor)
        {
            ChangeSet changes = null;
            if (query == null)
            {
                changes = await httpClient.GetChangesAsync(RemovePrefix(bucket.BucketName), DownloadBatchSize, anchor, uploadAnchor);
            }
            else
            {
                changes = await httpClient.GetChangesAsync(RemovePrefix(bucket.BucketName), query, DownloadBatchSize, anchor,uploadAnchor);
            }
            return changes;
        }
#endif

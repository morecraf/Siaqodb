...
namespace Microsoft.Synchronization.Services.Formatters
{
    static class FormatterUtilities
    {
        ...
        {
            return date.ToString(FormatterConstants.AtomDateTimeLexicalRepresentation, FormatterConstants.USCultureInfo);
        }
        ...
        {
            return dto.ToString(FormatterConstants.AtomDateTimeOffsetLexicalRepresentation, FormatterConstants.USCultureInfo);
        }
        ...
        public static string ConvertDateTimeToJson(DateTime date)
        {
            ...
            return string.Format(FormatterConstants.USCultureInfo, 
        ...
        {
            return string.Format(FormatterConstants.USCultureInfo, FormatterConstants.JsonTimeFormat, t.ToString());
        }
        ...
        public static string ConvertDateTimeOffsetToJson(DateTimeOffset dto)
        {
            return string.Format(FormatterConstants.USCultureInfo, 
                                 ...
                                 dto.ToString(FormatterConstants.JsonDateTimeOffsetLexicalRepresentation, FormatterConstants.USCultureInfo));
        }
        ...
        internal static object ParseDateTimeFromString(string value, Type type)
        {
            ...
            {
                throw new InvalidOperationException(string.Format(FormatterConstants.USCultureInfo, "Invalid Date/Time value received. Unable to parse value {0} to type {1}.", value, type.Name));
            }
        }
        ...
        private static object ParseJsonString(string value, Type type)
        {
            if (FormatterConstants.DateTimeType.IsAssignableFrom(type))
            {
                try
                {
                    ...
                    long ticks = long.Parse(ticksStr, FormatterConstants.USCultureInfo) * FormatterConstants.JsonNanoToMilliSecondsFactor +
                ...
                {
                    throw new InvalidOperationException(string.Format(FormatterConstants.USCultureInfo, @"Invalid Json DateTime value received. Value {0} is not in format '\/Date(ticks)\/'.", value));
                }
            }
            else if (FormatterConstants.DateTimeOffsetType.IsAssignableFrom(type))
            {
                ...
                {
                    throw new InvalidOperationException(string.Format(FormatterConstants.USCultureInfo, @"Invalid Json DateTimeOffset value received. Value {0} is not in format 'datetimeoffset'yyyy-MM-ddTHH:mm:ss.fffffffzzz''.", value));
                }
            }
            else
            {
                ...
                {
                    throw new InvalidOperationException(string.Format(FormatterConstants.USCultureInfo, @"Invalid Json TimeSpan value received. Value {0} is not in format 'time'HH:mm:ss''.", value));
                }

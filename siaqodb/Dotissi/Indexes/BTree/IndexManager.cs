...
namespace Dotissi.Indexes
{
    class IndexManager
    {
        ...
        public bool LoadOidsByIndex(SqoTypeInfo ti, string fieldName, Where where, List<int> oids)
        {
            if (cacheIndexes != null)
            {
                ...
                if (index != null && where.OperationType != OperationType.Contains && where.OperationType != OperationType.NotEqual && where.OperationType != OperationType.EndWith)
                {
                    ...
                    {
                        Sqo.SiaqodbConfigurator.LogMessage("Index has corrupted, will be rebuild", Sqo.VerboseLevel.Info);
       ...
       public IBTree GetIndex(FieldSqoInfo finfo, SqoTypeInfo tinfo)
       {
           ...
           {
               Sqo.SiaqodbConfigurator.LogMessage("IndexInfo cannot be loaded, index will be rebuild", Sqo.VerboseLevel.Info);
           }
                           ...
                           if (oldValuesOfIndexedFields.ContainsKey(fi.Name))//update occur
                           {
                               ...
                               else
                               {
                                   ...
                                   if (fieldType.IsEnum())
                                   {
                                       ...
                                       currentFieldVal = Sqo.Utilities.Convertor.ChangeType(objInfo.AtInfo[fi], enumType);
                                   }
                   ...
                   if (index != null)
                   {
                       ...
                       else
                       {
                           ...
                           if (fieldType.IsEnum())
                           {
                               ...
                               currentFieldVal = Sqo.Utilities.Convertor.ChangeType(newValue, enumType);
                           }

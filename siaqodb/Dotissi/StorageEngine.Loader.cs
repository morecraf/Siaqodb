...
namespace Dotissi
{
    partial class StorageEngine
    {
        ...
        internal ObjectList<T> LoadAll<T>(SqoTypeInfo ti)
        {
            ...
            int rangeSize = Convert.ToInt32((Sqo.SiaqodbConfigurator.BufferingChunkPercent * nrRecords / 100));
            ...
            for (int i = 0; i < nrRecords; i++)
            {
                ...
                if (Sqo.SiaqodbConfigurator.RaiseLoadEvents)
                {
                    Sqo.LoadingObjectEventArgs args = new Sqo.LoadingObjectEventArgs(oid, ti.Type);
                ...
                {
                    Sqo.SiaqodbConfigurator.LogMessage("Object with OID:"+oid.ToString()+" seems to be corrupted!", Sqo.VerboseLevel.Error);
                 
                    if (SiaqodbUtil.IsRepairMode)
                    {
                        Sqo.SiaqodbConfigurator.LogMessage("Object with OID:" + oid.ToString() + " is deleted", Sqo.VerboseLevel.Warn);
        ...
        internal ObjectList<object> LoadAllByType(SqoTypeInfo ti)
        {
            ObjectList<object> ol = new ObjectList<object>();
            ObjectSerializer serializer = SerializerFactory.GetSerializer(this.path, GetFileByType(ti), useElevatedTrust);
            serializer.NeedReadComplexObject += new EventHandler<ComplexObjectEventArgs>(serializer_NeedReadComplexObject);
            serializer.NeedCacheDocument += new EventHandler<DocumentEventArgs>(serializer_NeedCacheDocument);
            int nrRecords = ti.Header.numberOfRecords;
            ...
            for (int i = 0; i < nrRecords; i++)
            {
                ...
                try
                {
                    serializer.ReadObject(currentObj, ti, oid, this.rawSerializer);
                }
                catch (ArgumentException ex)
                {
                    Sqo.SiaqodbConfigurator.LogMessage("Object with OID:" + oid.ToString() + " seems to be corrupted!", Sqo.VerboseLevel.Error);
        ...
        internal List<int> LoadFilteredOids(Where where)
        {
            ...
            for (int j = (where.AttributeName.Count - 1); j >= 0; j--)
            {
                ...
                FieldInfo finfo = Dotissi.Meta.MetaExtractor.FindField(type, fieldName);
                if (finfo != null )
                {
...
#endif
        internal Sqo.IObjectList<T> LoadByOIDs<T>(List<int> oids, SqoTypeInfo ti)
        {
            ObjectList<T> ol = new ObjectList<T>();
            ObjectSerializer serializer = SerializerFactory.GetSerializer(this.path, GetFileByType(ti), useElevatedTrust);
            serializer.NeedReadComplexObject += new EventHandler<ComplexObjectEventArgs>(serializer_NeedReadComplexObject);
...
#if ASYNC_LMDB
        internal async Task<Sqo.IObjectList<T>> LoadByOIDsAsync<T>(List<int> oids, SqoTypeInfo ti)
        {
            ObjectList<T> ol = new ObjectList<T>();
            ObjectSerializer serializer = SerializerFactory.GetSerializer(this.path, GetFileByType(ti), useElevatedTrust);
            serializer.NeedReadComplexObjectAsync += new ComplexObjectEventHandler(serializer_NeedReadComplexObjectAsync);
        ...
        void serializer_NeedReadComplexObject(object sender, ComplexObjectEventArgs e)
        {
            ...
            else if (e.ParentType != null && Sqo.SiaqodbConfigurator.LazyLoaded != null && Sqo.SiaqodbConfigurator.LazyLoaded.ContainsKey(e.ParentType))
            {
                if (Sqo.SiaqodbConfigurator.LazyLoaded[e.ParentType])
                {
        ...
        internal object LoadObjectByOID(SqoTypeInfo ti, int oid, List<string> includes)
        {
            ...
            foreach (string path in includes)
            {
                ...
                foreach (var include in arrayPath)
                {
                    ...
                    string backingField = Dotissi.Utilities.ExternalMetaHelper.GetBackingField(property);
        ...
        internal async Task<bool> IsObjectDeletedAsync(int oid, SqoTypeInfo ti)
        {
         ...
         internal List<int> LoadOidsByField(SqoTypeInfo ti, string fieldName, object obj)
         {
             ...
             FieldSqoInfo fi = Dotissi.Utilities.MetaHelper.FindField(ti.Fields, fieldName);
             if (fi == null)
             {

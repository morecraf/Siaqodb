    ...
    partial class ObjectSerializer
    {
        public void ReadObject(object obj, SqoTypeInfo ti, int oid, RawdataSerializer rawSerializer)
        {
            ...
            foreach (FieldSqoInfo ai in ti.Fields)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Sqo.SiaqodbConfigurator.LogMessage("Field's" + ai.Name + " value of Type " + ti.TypeName + "cannot be loaded, will be set to default.", Sqo.VerboseLevel.Info);
                    fieldVal = MetaHelper.GetDefault(ai.AttributeType);
                }
                if (ai.AttributeTypeId == Dotissi.Meta.MetaExtractor.documentID)
                { 
                    ...
                    if (dinfo != null)
                    {
                        ...
                        fieldVal = Sqo.SiaqodbConfigurator.DocumentSerializer.Deserialize(ai.AttributeType, dinfo.Document);
        ...
        public object ReadFieldValue(SqoTypeInfo ti, int oid, FieldSqoInfo fi, RawdataSerializer rawSerializer)
        {
            ...
            catch (Exception ex)
            {
                ...
                Sqo.SiaqodbConfigurator.LogMessage("Field's" + fi.Name + " value of Type " + ti.TypeName + "cannot be loaded,will be set to default.", Sqo.VerboseLevel.Info);
        ...
        internal void ReadObjectRow(Sqo.Utilities.ObjectRow row, SqoTypeInfo ti, int oid, RawdataSerializer rawSerializer)
        {
            ...
            {
                // long position = (long)ti.Header.headerSize + (long)((long)(oid - 1) * (long)ti.Header.lengthOfRecord);
                long position = Dotissi.Utilities.MetaHelper.GetSeekPosition(ti, oid);
                int recordLength = ti.Header.lengthOfRecord;
                byte[] b = new byte[recordLength];
                if (oidStart == 0 && oidEnd == 0)
                {
                ...
                {
                    byte[] field = GetFieldBytes(b, ai.Header.PositionInRecord, ai.Header.Length);
                    if (typeof(IList).IsAssignableFrom(ai.AttributeType) || ai.IsText || ai.AttributeTypeId == Dotissi.Meta.MetaExtractor.complexID || ai.AttributeTypeId==Dotissi.Meta.MetaExtractor.dictionaryID || ai.AttributeTypeId==Dotissi.Meta.MetaExtractor.documentID)
                    {
                        row[ai.Name] = field;
                    }
                    else
...
#endif
        internal Sqo.Utilities.ATuple<int, int> GetArrayMetaOfField(SqoTypeInfo ti, int oid, FieldSqoInfo fi)
        {
            long position = MetaHelper.GetSeekPosition(ti, oid);
            int recordLength = ti.Header.lengthOfRecord;
            if (fi == null)
            ...
            Array.Copy(bytes, Dotissi.Meta.MetaExtractor.ExtraSizeForArray - 4, nrElemeBytes, 0, 4);
            int nrElem = (int)ByteConverter.DeserializeValueType(typeof(int), nrElemeBytes, ti.Header.version);

            return new Sqo.Utilities.ATuple<int, int>(rawInfoOID, nrElem);

        }
        #if ASYNC_LMDB
        internal async Task<Sqo.Utilities.ATuple<int, int>> GetArrayMetaOfFieldAsync(SqoTypeInfo ti, int oid, FieldSqoInfo fi)
        {
            long position = MetaHelper.GetSeekPosition(ti, oid);
            int recordLength = ti.Header.lengthOfRecord;
            if (fi == null)

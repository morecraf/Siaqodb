        ...
        private SqoTypeInfo DeserializeSqoTypeInfoFromBuffer(byte[] readFullSqoTypeInfo, bool loadRealType)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < tInfo.Header.NrFields; i++)
                {
                    ...
                    int currentPositionField = (i * Dotissi.Meta.MetaExtractor.FieldSize) + currentPosition + 4;
                    ...
                    if (loadRealType)
                    {
                        ...
                        ai.FInfo = Dotissi.Meta.MetaExtractor.FindField(tInfo.Type, ai.Name);
                        Dotissi.Meta.MetaExtractor.FindAddConstraints(tInfo, ai);
                        Dotissi.Meta.MetaExtractor.FindAddIndexes(tInfo, ai);

                    }
                    if (ai.AttributeTypeId == Dotissi.Meta.MetaExtractor.complexID || ai.AttributeTypeId == Dotissi.Meta.MetaExtractor.dictionaryID || ai.AttributeTypeId==Dotissi.Meta.MetaExtractor.documentID)
                    {
                        if (loadRealType)
                        {
                    ...
                    else if (ai.Header.Length - 1 == Dotissi.Meta.MetaExtractor.GetSizeOfField(ai.AttributeTypeId))//is Nullable<>
                    {
                        Type fGen = typeof(Nullable<>);
                        ai.AttributeType = fGen.MakeGenericType(Cache.Cache.GetTypebyID(ai.AttributeTypeId));
                    }
                    else if (Dotissi.Meta.MetaExtractor.IsTextType(ai.AttributeTypeId))
                    {
                        ai.AttributeType = typeof(string);
                        ai.IsText = true;
                    }
                    else if (ai.AttributeTypeId > Dotissi.Meta.MetaExtractor.ArrayTypeIDExtra)//is IList<> or Array
                    {
                        if (loadRealType)
                        {
                            ai.AttributeType = ai.FInfo.FieldType;
                        ...
                        {
                            if (ai.AttributeTypeId - Dotissi.Meta.MetaExtractor.ArrayTypeIDExtra == Dotissi.Meta.MetaExtractor.complexID
                                || ai.AttributeTypeId - Dotissi.Meta.MetaExtractor.ArrayTypeIDExtra == Dotissi.Meta.MetaExtractor.jaggedArrayID)
                            {
                        ...
                        if (ai.AttributeTypeId - Dotissi.Meta.MetaExtractor.ArrayTypeIDExtra == Dotissi.Meta.MetaExtractor.textID)
                        {
                            ai.IsText = true;
                        }
                           
                    }
                    else if (ai.AttributeTypeId > Dotissi.Meta.MetaExtractor.FixedArrayTypeId)//is IList<> or Array
                    {
                        if (loadRealType)
                        {
                            ai.AttributeType = ai.FInfo.FieldType;
                        ...
                        {
                            if (ai.AttributeTypeId - Dotissi.Meta.MetaExtractor.FixedArrayTypeId == Dotissi.Meta.MetaExtractor.complexID
                                || ai.AttributeTypeId - Dotissi.Meta.MetaExtractor.FixedArrayTypeId == Dotissi.Meta.MetaExtractor.jaggedArrayID)
                            {
            ...
            {
                Sqo.SiaqodbConfigurator.LogMessage("File:" + this.filePath + " is not a valid Siaqodb database file,skipped; error:"+ex.ToString(), Sqo.VerboseLevel.Info);
            }
            return null;
        }

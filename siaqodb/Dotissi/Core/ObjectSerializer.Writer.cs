...
namespace Dotissi.Core
{
    partial class ObjectSerializer
    {
        ...
        internal byte[] GetObjectBytes(ObjectInfo oi, RawdataSerializer rawSerializer)
        {
            ...
            {
                byte[] by = null;
                if (ai.AttributeTypeId == Dotissi.Meta.MetaExtractor.complexID || ai.AttributeTypeId == (Dotissi.Meta.MetaExtractor.ArrayTypeIDExtra + Dotissi.Meta.MetaExtractor.complexID) || ai.AttributeTypeId==Dotissi.Meta.MetaExtractor.documentID)
                {
        ...
        public byte[] GetComplexObjectBytes(object obj,bool returnOnlyOID_TID)
        {
            ...
            this.OnNeedSaveComplexObject(args);
            byte[] by = new byte[Dotissi.Meta.MetaExtractor.GetAbsoluteSizeOfField(Dotissi.Meta.MetaExtractor.complexID)];
        ...
        public async Task<byte[]> GetComplexObjectBytesAsync(object obj, bool returnOnlyOID_TID)
        {
            ComplexObjectEventArgs args = new ComplexObjectEventArgs(obj, returnOnlyOID_TID);
            await this.OnNeedSaveComplexObjectAsync(args).ConfigureAwait(false);
            byte[] by = new byte[Dotissi.Meta.MetaExtractor.GetAbsoluteSizeOfField(MetaExtractor.complexID)];
            byte[] complexOID = ByteConverter.IntToByteArray(args.SavedOID);
            byte[] complexTID = ByteConverter.IntToByteArray(args.TID);
            Array.Copy(complexOID, 0, by, 0, complexOID.Length);
            Array.Copy(complexTID, 0, by, 4, complexTID.Length);
        ...
        internal bool SaveFieldValue(int oid, string field, SqoTypeInfo ti, object value, RawdataSerializer rawSerializer)
        {
            ...
            else if (value != null && ai.AttributeType != value.GetType())
            {
                try
                {
                    ...
                    object valConvert = Sqo.Utilities.Convertor.ChangeType(value, ai.AttributeType);
                ...
                catch
                {
                    ...
                    Sqo.SiaqodbConfigurator.LogMessage(msg,Sqo.VerboseLevel.Error);
        ...
        internal void SaveObjectTable(SqoTypeInfo actualTypeinfo, SqoTypeInfo oldSqoTypeInfo, Sqo.Utilities.ObjectTable table, RawdataSerializer rawSerializer)
        {
            ...
            foreach (Sqo.Utilities.ObjectRow row in table.Rows)
            {
                ...
                foreach (FieldSqoInfo ai in actualTypeinfo.Fields)
                {
                    ...
                    {
                        if (ai.AttributeTypeId == Dotissi.Meta.MetaExtractor.complexID || ai.AttributeTypeId==Dotissi.Meta.MetaExtractor.documentID)
                        {
                            fieldVal = null;
                        }
                    ...
                    if (joinedFields[ai] != null) //existed in old Type
                    {
                        ...
                        {
                            if (typeof(IList).IsAssignableFrom(ai.AttributeType) || ai.AttributeTypeId == Dotissi.Meta.MetaExtractor.dictionaryID || joinedFields[ai].AttributeTypeId==Dotissi.Meta.MetaExtractor.dictionaryID)
                            {
                                throw new TypeChangedException("Change array or dictionary type it is not supported");
                            }
                            else
                            {
                                fieldVal = Sqo.Utilities.Convertor.ChangeType(fieldVal, ai.AttributeType);
                            }
                        }
                    }
                    if (ai.AttributeTypeId == Dotissi.Meta.MetaExtractor.complexID || ai.AttributeTypeId==Dotissi.Meta.MetaExtractor.documentID)
                    ...
                    else if (ai.AttributeTypeId == Dotissi.Meta.MetaExtractor.dictionaryID)
                    {
...
#if ASYNC_LMDB
        internal async Task SaveObjectTableAsync(SqoTypeInfo actualTypeinfo, SqoTypeInfo oldSqoTypeInfo, Sqo.Utilities.ObjectTable table, RawdataSerializer rawSerializer)
        {
            Dictionary<FieldSqoInfo, FieldSqoInfo> joinedFields = JoinFieldsSqoInfo(actualTypeinfo, oldSqoTypeInfo);

    ...
    internal static class SqoQueryExtensionsImpl
    {
        ...
        public static ISqoQuery<TSource> Where<TSource>(ISqoQuery<TSource> self, Expression<Func<TSource, bool>> expression)
        {
            ...
            try
            {
                ...
                if (SqoQuery == null)
                    throw new Sqo.Exceptions.LINQUnoptimizeException();
            ...
            catch (Sqo.Exceptions.LINQUnoptimizeException)
            {
                Sqo.SiaqodbConfigurator.LogMessage("Expression:" + expression.ToString() + " cannot be parsed, query runs un-optimized!",VerboseLevel.Warn);
		...
		public static ISqoQuery<TRet> Select<TSource, TRet>(ISqoQuery<TSource> self, Expression<Func<TSource, TRet>> selector)
		{
			...
			try
			{
				...
				if (to == null )
				{
					throw new Sqo.Exceptions.LINQUnoptimizeException("MultiJoint not yet supported");
				}
			...
			catch (Sqo.Exceptions.LINQUnoptimizeException ex3)
            {
                Sqo.SiaqodbConfigurator.LogMessage("Expression:" + selector.ToString() + " cannot be parsed, query runs un-optimized!", VerboseLevel.Warn);
				#if (WP7 || UNITY3D) && !MANGO && !XIOS
                Func<TSource, TRet> fn = (Func<TSource, TRet>)ExpressionCompiler.ExpressionCompiler.Compile(selector);
#else
		...
		public static ISqoQuery<TResult> Join<TOuter, TInner, TKey, TResult>(ISqoQuery<TOuter> outer, IEnumerable<TInner> inner, Expression<Func<TOuter, TKey>> outerKeySelector, Expression<Func<TInner, TKey>> innerKeySelector, Expression<Func<TOuter, TInner, TResult>> resultSelector)
		{
            ...
            try
            {
                ...
                {
                    throw new Sqo.Exceptions.LINQUnoptimizeException("MultiJoin not yet supported");
                }
            ...
            {
                Sqo.SiaqodbConfigurator.LogMessage("Expression:" + resultSelector.ToString() + " cannot be parsed, query runs un-optimized!", VerboseLevel.Warn);
				#if (WP7 || UNITY3D) && !MANGO && !XIOS 
        ...
        public static ISqoOrderedQuery<TSource> OrderBy<TSource, TKey>(ISqoQuery<TSource> source, Expression<Func<TSource, TKey>> keySelector)
        {
            ...
            if (r != null && sqoQuery != null)
            {
                ...
                Sqo.Utilities.SqoComparer<Sqo.Utilities.SqoSortableItem> comparer = new SqoComparer<SqoSortableItem>(false);
                SqoOrderedQuery<TSource> orderedQuery = new SqoOrderedQuery<TSource>(sqoQuery.Siaqodb, orderedList, comparer);


                return orderedQuery;
            }
            Sqo.SiaqodbConfigurator.LogMessage("Expression:" + keySelector.ToString() + " cannot be parsed, query runs un-optimized!", VerboseLevel.Warn);
	#if (WP7 || UNITY3D) && !MANGO && !XIOS

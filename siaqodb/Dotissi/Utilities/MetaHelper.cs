...
namespace Dotissi.Utilities
{
    class MetaHelper
    {
        ...
        { 
             ...
             if (Sqo.SiaqodbConfigurator.PropertyMaps != null)
             {
                 if (Sqo.SiaqodbConfigurator.PropertyMaps.ContainsKey(mi.DeclaringType))
                 {
                     if (Sqo.SiaqodbConfigurator.PropertyMaps[mi.DeclaringType].ContainsKey(mi.Name))
                     {
                         return Sqo.SiaqodbConfigurator.PropertyMaps[mi.DeclaringType][mi.Name];
                     }
                 }
             }
             return null;
        ...
        {
            FieldInfo fi = Dotissi.Meta.MetaExtractor.FindField(ti, "tickCount");
            if (fi != null)
            {
        ...
        {
            int typeId = Dotissi.Meta.MetaExtractor.GetAttributeType(ty);

            return Dotissi.Meta.MetaExtractor.GetAbsoluteSizeOfField(typeId);
        }
        ...
        internal static string GetFieldAsInDB(string field, Type type)
        {
            ...
            Dotissi.Meta.MetaExtractor.FindFields(fi, automaticProperties, type);
...
#if SILVERLIGHT
            if (!Sqo.SiaqodbConfigurator.UseLongDBFileNames)
            {

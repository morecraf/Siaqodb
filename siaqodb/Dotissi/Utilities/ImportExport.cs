    ...
    class ImportExport
    {
        ...
        public static void ExportToXML<T>(System.Xml.XmlWriter writer, IList<T> objects, Siaqodb siaqodb) 
        {
            ...
            foreach (T obj in objects)
            {
                ...
                ObjectInfo oi = Dotissi.Meta.MetaExtractor.GetObjectInfo(obj, ti,siaqodb.metaCache);
                foreach (FieldSqoInfo fi in ti.Fields)
                {
                    ...
                    else if (typeElement.IsEnum())
                    {
                        ...
                        object realObject = Sqo.Utilities.Convertor.ChangeType(oi.AtInfo[fi], enumType);
        ...
        {
            Sqo.Utilities.ObjectTable obTable = new Sqo.Utilities.ObjectTable();
            reader.Read();
            reader.ReadStartElement("SiaqodbObjects");
            SqoTypeInfo ti = siaqodb.GetSqoTypeInfo<T>();
            bool colFinish = false;
            Sqo.Utilities.ObjectRow currentRow = null;
            int index = 0;
        ...
        private static void ReadMemberValue(Sqo.Utilities.ObjectRow currentRow, System.Xml.XmlReader reader, int index, Dictionary<int, Type> members)
        {
            ...
            if (!reader.IsEmptyElement)
            {
                ...
                else if (members[index].IsEnum())
                {
                    ...
                    object realObject = Sqo.Utilities.Convertor.ChangeType(s, enumType);

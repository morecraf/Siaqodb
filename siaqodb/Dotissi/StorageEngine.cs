...
namespace Dotissi
{
    ...
    partial class StorageEngine
    {
        ...
        List<Sqo.Utilities.ATuple<Type, String>> includePropertiesCache;
...
#if SILVERLIGHT
            if (!Sqo.SiaqodbConfigurator.UseLongDBFileNames && !fileName.StartsWith("Sqo.Indexes.BTreeNode"))
            {
                fileName = fileName.GetHashCode().ToString();
            }
#endif
        ...
        internal void LoadMetaDataTypes()
        {
            ...
            {
                this.UpgradeInternalSqoTypeInfos(typeof(Sqo.MetaObjects.RawdataInfo), "rawdatainfo", false);
                string indexinfoName = MetaHelper.GetOldFileNameByType(typeof(Dotissi.Indexes.IndexInfo2));
                if (MetaHelper.FileExists(this.path, indexinfoName,this.useElevatedTrust))
                {
                    this.UpgradeInternalSqoTypeInfos(typeof(Dotissi.Indexes.IndexInfo2), "indexinfo2", true);
                }
            }
...
#endif
            {
                ...
                CacheCustomFileNames.AddFileNameForType(new SqoTypeInfo(typeof(Sqo.MetaObjects.RawdataInfo)).TypeName, "rawdatainfo", false);
                CacheCustomFileNames.AddFileNameForType(new SqoTypeInfo(typeof(Dotissi.Indexes.IndexInfo2)).TypeName, "indexinfo2", false);
#if KEVAST
                ...
                seralizer = SerializerFactory.GetSerializer(path, this.GetFileByType(new SqoTypeInfo(typeof(Dotissi.Indexes.IndexInfo2)).TypeName), this.useElevatedTrust);
        ...
        internal async Task LoadMetaDataTypesAsync()
        {
            ...
            if (MetaHelper.FileExists(this.path, rawdatainfoName, this.useElevatedTrust))
            {
                ...
                if (MetaHelper.FileExists(this.path, indexinfoName, this.useElevatedTrust))
                {
                    await this.UpgradeInternalSqoTypeInfosAsync(typeof(Dotissi.Indexes.IndexInfo2), "indexinfo2", true).ConfigureAwait(false);
                }
            }
        ...
        {
            SqoTypeInfo actualType = Dotissi.Meta.MetaExtractor.GetSqoTypeInfo(ti.Type);
            if (!Dotissi.Meta.MetaExtractor.CompareSqoTypeInfos(actualType, ti))
            {
                Sqo.Utilities.ObjectTable table = this.LoadAll(ti);
                ...
                {
                    Sqo.SiaqodbConfigurator.LogMessage("Type:" + ti.Type.ToString() + " cannot be upgraded, will be marked as 'Old'!",Sqo.VerboseLevel.Error);
                    ti.IsOld = true;
                    this.SaveType(ti);
        ...
        private async Task CompareSchemaAsync(SqoTypeInfo ti)
        {
            ...
            if (!Dotissi.Meta.MetaExtractor.CompareSqoTypeInfos(actualType, ti))
            {
                Sqo.Utilities.ObjectTable table = await this.LoadAllAsync(ti).ConfigureAwait(false);
        ...
        private void UpgradeInternalSqoTypeInfos(Type type, string newName, bool dropIt)
        {
            ...
            if (ti != null)
            {
                ...
                {
                    this.DropType(ti);
                    SqoTypeInfo actualType1 = Dotissi.Meta.MetaExtractor.GetSqoTypeInfo(ti.Type);
            ...
            lock (_syncRoot)
            {
                ...
                if (headers.Count > 0)
                {
                    ...
                    foreach (TransactionObjectHeader header in headers)
                    {
                        ...
                        {
                            tiObject = Dotissi.Meta.MetaExtractor.GetSqoTypeInfo(objType);
                        }
                ...
                foreach (TransactionTypeHeader tHeader in tHeaders)
                {
                    ...
                    {
                        ti = Dotissi.Meta.MetaExtractor.GetSqoTypeInfo(t);
                    }
                    ObjectSerializer serializer = SerializerFactory.GetSerializer(this.path, GetFileByType(ti), useElevatedTrust);
        ...
        internal void RollbackObject(object oi, SqoTypeInfo ti)
        {
            ...
            ObjectInfo objInfo = Dotissi.Meta.MetaExtractor.GetObjectInfo(oi, ti,metaCache);
        ...
        {
            ObjectInfo objInfo = Dotissi.Meta.MetaExtractor.GetObjectInfo(obj, ti, metaCache);
...
#if UNITY3D || CF || MONODROID
#else
        protected  void OnIndexesSaveAsyncFinished(Sqo.IndexesSaveAsyncFinishedArgs e)
        {

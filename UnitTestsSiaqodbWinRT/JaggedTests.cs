        ...
        public async Task TestStoreDictionaryTransactional()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < 10; i++)
                {
                    ...
                    if (i == 5)
                    {
                        throw new Exception("need some RollbackAsync");
                    }
                }
                await transaction.CommitAsync();
            }
            catch (Exception ex)
            {
                ...
                if (ex.Message == "need some RollbackAsync")
                {
                    mustRollbackAsync = true;
                }
                else throw ex;

            }
            if (mustRollbackAsync)
            {
